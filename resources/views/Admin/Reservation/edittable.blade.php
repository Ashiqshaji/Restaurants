    ...
    <section id="assigntable" class="assigntable">
        ...
        <div class="container">
            <div class="row ">
                ...
                <div class="col-12">
                    <form id="table_update_remove" action="{{ route('admin.removetable') }}" method="POST">
                        @csrf
                        <div class="row mt-4">
                            <div class="col-6">
                                <h5> <span class="header"> Please choose the remove table</span> </h5>
                                {{-- <span class="selected-table"> Please choose the remove table </span> --}}

                            </div>
                            <div class="col-6">
                                <input type="hidden" id="table_id_update" name="table_id_update"
                                    value="{{ $data_user->table_id }}">
                        ...
                        <div class="row">
                            ...
                            <div class="col-12">
                                <div class="row">
                                    @foreach ($data_user_selected as $result)
                                        @php
                                            $colorClass = $result->table_id == $data_user->table_id ? '' : '';
                                            // $isSelectable = $result->table_id != $data_user->table_id;
                                        @endphp
                                        <div class="col-2">
                                            <div class="card mt-3 {{ $colorClass }}"
                                                id="cardupdate{{ $result->table_id }}">
                                                <div class="card-body">

                                                    {{-- @if ($isSelectable) --}}
                                                    <input class="form-check-input" type="checkbox"
                                                        name="selected_item_update[]" value="{{ $result->table_id }}"
                                                        id="itemupdate{{ $result->table_id }}"
                                                        onclick="toggleCardUpdate('{{ $result->table_id }}')"
                                                        style="display:none">
                                                    {{-- @endif --}}
                                                    <label for="item_update{{ $result->table_id }}">
                                                        <h4 class="card-title">{{ htmlspecialchars($result->table_no) }}
                                                        </h4>
                                                    </label>
                                                </div>
                                            </div>
                ...
                <div class="col-12">
                    <form id="table_confirm" action="{{ route('admin.selectiontable') }}" method="POST">
                        @csrf
                        <div class="row mt-4">
                            <div class="col-6">
